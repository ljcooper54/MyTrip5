...
// Provides:
// - Pexels iconic travel photo URL(s) (25s timeouts)
...
final class OpenAIImageClient {
    ...
    /// NEW: Fetch multiple Pexels candidates for an iconic image (used for cycling).
    func searchPexelsIconicImages(locationName: String, perPage: Int = 5) async throws -> [URL] {
        ...
        guard !decoded.photos.isEmpty else {
            throw NSError(domain: "Pexels", code: 404, userInfo: [NSLocalizedDescriptionKey: "No photo found for \(query)"])
        } // End guard photos

        var out: [URL] = []
        out.reserveCapacity(decoded.photos.count)

        for photo in decoded.photos {
            guard let imageURL = photo.src.bestURL() else { continue } // End guard imageURL
            out.append(imageURL)
        ...
        return out
    } // End func searchPexelsIconicImages (long)

    /// Backward-compatible: returns the first Pexels match.
    func generateIconicImage(locationName: String) async throws -> URL {
        let urls = try await searchPexelsIconicImages(locationName: locationName, perPage: 1)
        return urls[0]
